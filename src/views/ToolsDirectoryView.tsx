...
export const ToolsDirectoryView: React.FC = () => {
  ...
  const [quickFilter, setQuickFilter] = useState<'all' | 'popular' | 'top-rated' | 'free' | 'interactive'>('all');
  ...
  const filteredTools = useMemo(() => {
    return tools.filter((tool) => {
      ...
      if (quickFilter === 'popular') {
        ...
        if (tool.pricingType !== 'free' && tool.pricingType !== 'freemium') return false;
      } else if (quickFilter === 'interactive') {
        const isInteractive = ['ai-text-summarizer', 'ai-paraphrasing-tool', 'ai-email-writer', 'ai-title-generator', 'ai-meta-description-generator'].includes(tool.slug);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Featured Collection Callout Banner */}
      <div className="flex items-center justify-between flex-wrap gap-3 p-3.5 sm:p-4 rounded-2xl bg-gradient-to-r from-blue-500/10 via-indigo-500/10 to-transparent border border-blue-200/80 dark:border-blue-900/40 text-xs">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="font-bold text-slate-900 dark:text-white">Looking for Google's AI models & developer tools?</span>
            <span className="text-slate-500 dark:text-slate-400 ml-1.5 hidden sm:inline">Explore Gemini, Google AI Studio, Imagen 3, Veo, and NotebookLM in our dedicated collection.</span>
          </div>
        </div>
        <button
          onClick={() => navigate('/google-ai-tools')}
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold transition-all cursor-pointer shadow-2xs"
        >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 sm:p-5 shadow-xs space-y-3">
        ...
        {/* Quick Filter Chips & Secondary Use Case Filter */}
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100 dark:border-slate-800 text-xs">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1.5">
              <span className="text-slate-400 font-medium">Use Case:</span>
              <select
            ...
            {(selectedCategory !== 'all' || selectedPricing !== 'all' || selectedUseCase !== 'all' || quickFilter !== 'all' || searchTerm || showSavedOnly) && (
              <button
                onClick={resetFilters}
                className="inline-flex items-center gap-1 text-xs font-semibold text-rose-600 hover:text-rose-700 dark:text-rose-400 dark:hover:text-rose-300 transition-colors cursor-pointer ml-2"
              >
        ...
        {filteredTools.length > 0 ? (
          <>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500 dark:text-slate-400">
              <span>
                Showing <strong className="text-slate-800 dark:text-slate-200">{Math.min((currentPage - 1) * itemsPerPage + 1, filteredTools.length)} - {Math.min(currentPage * itemsPerPage, filteredTools.length)}</strong> of{' '}
                <strong className="text-indigo-600 dark:text-indigo-400">{filteredTools.length.toLocaleString()}</strong> tools (from {tools.length.toLocaleString()} total vetted tools)
              </span>
            ...
            {totalPages > 1 && (
              <div className="flex flex-wrap items-center justify-center gap-1.5 pt-4">
                <button
                  ...
                  className={`px-2.5 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold inline-flex items-center gap-1 transition-colors ${
                    currentPage === 1
                ...
                <div className="flex items-center gap-1">
                  {visiblePages.map((page, idx) => {
                    if (page === '...') {
                      return (
                        <span key={`ellipsis-${idx}`} className="w-7 text-center text-xs text-slate-400 font-bold">
                          ...
                        </span>
                      );
                    }
