...
import { getAllArticleTags } from '../data/articlesData';
...
export const BlogView: React.FC = () => {
  ...
  const popularTags = useMemo(() => {
    return getAllArticleTags().slice(0, 10);
  }, []);
  ...
  const filteredArticles = useMemo(() => {
    return articles.filter((a) => {
      ...
      if (selectedTag) {
        if (!a.tags || !a.tags.some((t) => t.toLowerCase() === selectedTag.toLowerCase())) {
          return false;
        }
      }
      if (searchQuery.trim()) {
  ...
  const feedArticles = useMemo(() => {
    if (selectedCategory === 'all' && !selectedTag && !searchQuery.trim() && featuredArticle) {
      ...
      <div className="space-y-4 pt-2">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="relative sm:w-72 grow">
              <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-3" />
              <input
                type="text"
                placeholder="Search by topic, tool or tag..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-10 pr-4 py-2 text-xs sm:text-sm border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
              />
              {searchQuery && (
                <button
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-200/80 dark:border-amber-800/60 hover:bg-amber-100 dark:hover:bg-amber-900/40 transition-colors shadow-2xs"
            >
              <Rss className="w-3.5 h-3.5 text-amber-500" />
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 text-xs scrollbar-none">
          <span className="flex items-center gap-1 text-slate-400 dark:text-slate-500 font-semibold shrink-0">
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-medium whitespace-nowrap transition-colors cursor-pointer text-[11px] ${
                  isTagActive
                    ...
                    : 'bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 border border-slate-200/60 dark:border-slate-800'
                }`}
              >
                #{t.name} ({t.count})
              </button>
          ...
          <div className="flex items-center justify-between py-2 px-4 rounded-xl bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 text-xs text-slate-600 dark:text-slate-400">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span>
                ...
                {selectedTag && (
                  <>
                    {' '}• tag: <strong className="text-indigo-600 dark:text-indigo-400">#{selectedTag}</strong>
                  </>
                )}
            ...
            <button
              ...
              className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:underline cursor-pointer shrink-0 ml-2"
            >
              Reset Filters
            </button>
          </div>
